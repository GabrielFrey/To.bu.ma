...
import { chargebackQuerySchema, parseDateRange } from './schemas.js';

export async function registerAnalyticsRoutes(v1: FastifyInstance) {
  v1.get('/analytics/total', async (req) => analytics.totalSpend(orgId(req)));
  v1.get('/analytics/by-agent', async (req) => analytics.spendByAgent(orgId(req)));
  v1.get('/analytics/by-task', async (req) => analytics.spendByTask(orgId(req)));
  v1.get('/analytics/by-project', async (req) => analytics.spendByProject(orgId(req)));
  v1.get('/analytics/active-budgets', async (req) => analytics.activeBudgets(orgId(req)));
  v1.get('/analytics/warnings', async (req) => analytics.warnings(orgId(req)));
  v1.get('/analytics/blocked', async (req) => analytics.blockedRequests(orgId(req)));
  v1.get('/analytics/expensive-prompts', async (req) => analytics.expensivePrompts(orgId(req)));
  v1.get('/analytics/loops', async (req) => analytics.inefficientLoops(orgId(req)));
  v1.get('/analytics/recommendations', async (req) => analytics.recommendations(orgId(req)));
